...
export function ResultsActionButtons({ match }: { match: BenefitMatch }) {
  const officialUrl = getOfficialProgramUrl(match);
  const mapUrl = getMapSearchUrl(match);

  if (!officialUrl && !mapUrl) {
    return (
      <p className="rounded-2xl bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-950">
        Source link needs review before external actions can be shown.
      </p>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
      {officialUrl ? (
        ...
        <p className="rounded-xl bg-amber-50 px-4 py-3 text-xs font-bold leading-5 text-amber-950">
      ...
      {mapUrl ? (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#B8793A]/25 bg-white px-5 py-3 text-sm font-black text-[#244B35] transition hover:bg-[#F6F1E7]"
        >
...
export function getMapSearchUrl(match: BenefitMatch) {
  if (isValidExternalUrl(match.officeSearchUrl)) return match.officeSearchUrl ?? "";

  const query = buildSafeMapQuery(match);
...
function buildSafeMapQuery(match: BenefitMatch) {
  const officeType = match.location?.trim();
  if (!officeType) return "";

  return `${officeType} ${match.category}`.replace(/\s+/g, " ").trim();
}
